...
package org.blch.geom
{
	...
	public class Line2D
	{
		public var pointA:Vector2f;	// Endpoint A of our line segment
		public var pointB:Vector2f;	// Endpoint B of our line segment
		
		private var m_Normal:Vector2f;	// 'normal' of the ray. 
		private var right_Normal:Vector2f;
		...
		public function getVector2f():Vector2f{
			return new Vector2f(pointB.x-pointA.x,pointB.y-pointA.y);
		}
		...
		public function getNormal():Vector2f
		{
		...
		public function signedDistanceAdd(point:Vector2f,addreduce:Boolean):Vector2f
		{
			if (!m_NormalCalculated)
			{
				computeNormal();
			...
			var testVector:Vector2f;
			if(addreduce){
				testVector = new Vector2f(point.x + 50*m_Normal.x, point.y + 50*m_Normal.y);
			}else{
				testVector = new Vector2f(point.x - 50*m_Normal.x, point.y - 50*m_Normal.y);
			}
			return testVector;
		...
		public function getAngle(otherLine:Line2D):Number{
			var v1:Vector2f = new Vector2f(pointB.x-pointA.x,pointB.y-pointA.y);
			var v2:Vector2f = new Vector2f(otherLine.pointB.x-otherLine.pointA.x,otherLine.pointB.y-otherLine.pointA.y);
			var n:Number = v1.dot(v2)/(v1.length()*v2.length())
			return n;
		}
		public function getParallel():Line2D{
			right_Normal = getDirection();
			
			var oldYValue:Number = right_Normal.y;
			/* right_Normal.y = -right_Normal.x;
			right_Normal.x = oldYValue; */
			
			right_Normal.y = right_Normal.x;
			right_Normal.x = -oldYValue;
			
			right_Normal = right_Normal.mult(30);
			
			var pa:Vector2f = pointA.clone();
			var pb:Vector2f = pointB.clone();
			
			pa = pa.add(right_Normal);
			pb = pb.add(right_Normal);
			
			var dic:Vector2f = getDirection().mult(50);
			
			pa = pa.subtract(dic);
			pb = pb.add(dic);
			
			return new Line2D(pa,pb);
		} 
