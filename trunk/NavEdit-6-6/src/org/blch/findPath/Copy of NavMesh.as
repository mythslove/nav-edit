...
//				trace("&&&&&", st.parent);
		...
		private function getPath(start:Vector2f, end:Vector2f):Array {
			...
			trace("wayPoint起始坐标",wayPoint.position.x,wayPoint.position.y);
			while (true) {
				wayPoint = this.getFurthestWayPoint(wayPoint, cellPath, end);
				trace("wayPoint坐标",wayPoint.position.x,wayPoint.position.y);
				if(waypathArr[waypathArr.length-1].active){
					waypathArr.push(wayPoint);
				}else{
					fixedWayPoint(waypathArr[waypathArr.length-2],waypathArr[waypathArr.length-1],wayPoint);
					wayPoint = waypathArr[waypathArr.length-1];
					trace("wayPoint修改坐标",wayPoint.position.x,wayPoint.position.y);
					wayPoint.active = true;
				}
		...
		private function fixedWayPoint(first:WayPoint,second:WayPoint,third:WayPoint):void{
		...
		private function getFurthestWayPoint(wayPoint:WayPoint, cellPath:Vector.<Cell>, end:Vector2f):WayPoint {
			...
			for (var i:int=startIndex+1; i<cellPath.length; i++) {
				...
				if (i == cellPath.length-1) {
					...
					//testPtA = new Vector2f(outSide.getPointA().x - 30*outSide.getNormal().x,outSide.getPointA().y - 30*outSide.getNormal().y);
					//testPtB = new Vector2f(outSide.getPointA().x + 30*outSide.getNormal().x,outSide.getPointA().y + 30*outSide.getNormal().y);
				}
